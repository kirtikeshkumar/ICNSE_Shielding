...
void MyEventAction::GetPrimaryParticle(const G4Event *event) {
  ...
  // // Get the primary particle from the vertex (assumes one primary per
  // vertex) G4PrimaryParticle *primary = vertex->GetPrimary(); if (!primary) {
  ...
  // man->AddNtupleRow(2);
  // std::cout << name << " : " << energy << " : " << thetaDeg << std::endl;
#endif
}
