...
void MyPrimaryGenerator::CylinderVolumeSource(G4ThreeVector loc, G4double halfHt, G4double radius)
{
...
void MyPrimaryGenerator::GeneratePrimaries(G4Event *anEvent)
{
...
#ifdef SETUP_DECAY
  // std::vector<G4LogicalVolume *> NaILogical = fDetector->GetNaIVolumes();
  ...
  {
    G4Tubs *solid = dynamic_cast<G4Tubs *>(NaIPhysical[ij]->GetLogicalVolume()->GetSolid());
  ...
  if (particle == G4Geantino::Geantino() || particle->GetParticleName() == "Na22")
  {
    G4int Z = 11;
    G4int A = 22;
