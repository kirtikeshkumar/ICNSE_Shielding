...
MyDetectorConstruction::MyDetectorConstruction(std::string mat,
                                               std::string th) {
  // Here the materials are to be named by first letter of their names
  // The thicknesses will be fixed to 5 cm each
  // not used currently
  DefineMaterials();
...
void MyDetectorConstruction::ConstructSetup() {
  ...
  logicshellMuVetoOut =
      new G4LogicalVolume(shellMuVetoOut, myTolueneMat, "logicshellMuVetoOut");
  logicshellHDPEOuter =
      new G4LogicalVolume(shellHDPEOuter, HDPE, "logicshellHDPEOuter");
  ...
  logicshellHDPEInner =
      new G4LogicalVolume(shellHDPEInner, HDPE, "logicshellHDPEInner");
  logicshellMuVetoIn =
      new G4LogicalVolume(shellMuVetoIn, myTolueneMat, "logicshellMuVetoIn");
  logicshellCuColdBox =
      new G4LogicalVolume(shellCuColdBox, Copper, "logicshellCuColdBox");
  logicdetVol = new G4LogicalVolume(detVol, Vaccum, "logicdetVol");

  G4LogicalSkinSurface *skin1 =
      new G4LogicalSkinSurface("skin1", logicshellMuVetoOut, mirrorSurface);
  G4LogicalSkinSurface *skin2 =
      new G4LogicalSkinSurface("skin2", logicshellMuVetoIn, mirrorSurface);

  physshellMuVetoOut =
      new G4PVPlacement(0, G4ThreeVector(xloc, 0., 4.), logicshellMuVetoOut,
                        "physshellMuVetoOut", logicWorld, false, 1, true);
  physshellHDPEOuter =
      new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshellHDPEOuter,
                        "physshellHDPEOuter", logicWorld, false, 2, true);
  physshellBP = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshellBP,
                                  "physshellBP", logicWorld, false, 3, true);
  physshellLead =
      new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshellLead,
                        "physshellLead", logicWorld, false, 4, true);
  physshellHDPEInner =
      new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshellHDPEInner,
                        "physshellHDPEInner", logicWorld, false, 5, true);
  physshellMuVetoIn =
      new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshellMuVetoIn,
                        "physshellMuVetoIn", logicWorld, false, 6, true);
  physshellCuColdBox =
      new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshellCuColdBox,
                        "physshellCuColdBox", logicWorld, false, 7, true);
  physdetVol = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicdetVol,
                                 "physdetVol", logicWorld, false, 0, true);
}

void MyDetectorConstruction::ConstructSetupMod() {
  std::string name;
  detVol = new G4Box("Boxsensdet", detVolX * 0.5, detVolY * 0.5, detVolZ * 0.5);
  logicdetVol = new G4LogicalVolume(detVol, Vaccum, "logicdetVol");
  physdetVol = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicdetVol,
                                 "physdetVol", logicWorld, false, 0, true);
  double xsz = detVolX;
...
void MyDetectorConstruction::ConstructSetupV1() {
  ...
  // SubBoxSz is size of the box to be subtracted. the  x and y are to ensure
  // complete subtraction, z is since 4cm of muveto, and 10cm each of HDPE and
  // BP to be removed
  G4ThreeVector SubBoxSz = G4ThreeVector(151., 151., 24.);
  double zoffsetSubBox = -0.5 * (shellMuVetoOutSz.z() - SubBoxSz.z());
  // zoffset determines the position the steel table is compared to center of
  // table top part. there is 0.5*shellMuVetoOutSz.z() extent of
  // shellMuVetoOutSz below its center from this SubBoxSz.z() is removed the
  // steelplate center is 0.5*SteelPlateSz below its surface hence the following
  // formula
  double zoffset =
      0.5 * shellMuVetoOutSz.z() - SubBoxSz.z() + 0.5 * SteelPlateSz.z();
  G4ThreeVector offsetSubBox = G4ThreeVector(0., 0., zoffsetSubBox * cm);

  G4VSolid *shellMuVetoOut =
      SubtractBoxfrmShell(shellMuVetoOutSz, 4.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shell0 =
      SubtractBoxfrmShell(shell0Sz, 5.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shell1 =
      SubtractBoxfrmShell(shell1Sz, 5.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shell2 =
      SubtractBoxfrmShell(shell2Sz, 5.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shell3 =
      SubtractBoxfrmShell(shell3Sz, 5.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shell4 =
      SubtractBoxfrmShell(shell4Sz, 5.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shell5 =
      SubtractBoxfrmShell(shell5Sz, 5.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shell6 =
      SubtractBoxfrmShell(shell6Sz, 5.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shell7 =
      SubtractBoxfrmShell(shell7Sz, 5.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shellMuVetoIn =
      SubtractBoxfrmShell(shellMuVetoInSz, 4.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *shellCuColdBox =
      SubtractBoxfrmShell(shellCuColdBoxSz, 2.0, 0.0, SubBoxSz, offsetSubBox);
  G4VSolid *SteelPlate =
      new G4Box("SteelPlate", 0.5 * SteelPlateSz.x() * cm,
                0.5 * SteelPlateSz.y() * cm, 0.5 * SteelPlateSz.z() * cm);
  G4VSolid *UnderSideBP =
      new G4Box("UnderSideBP", 0.5 * UnderSideBPSz.x() * cm,
                0.5 * UnderSideBPSz.y() * cm, 0.5 * UnderSideBPSz.z() * cm);
  G4VSolid *UnderSideHDPE =
      new G4Box("UnderSideHDPE", 0.5 * UnderSideHDPESz.x() * cm,
                0.5 * UnderSideHDPESz.y() * cm, 0.5 * UnderSideHDPESz.z() * cm);
  detVol = new G4Box("Boxsensdet", 25. * cm, 25. * cm, 37.5 * cm);
  ...
  // logicshellMuVetoIn 	= new
  // G4LogicalVolume(shellMuVetoIn,myTolueneMat,"logicshellMuVetoIn");
  logicshellMuVetoIn =
      new G4LogicalVolume(shellMuVetoIn, Vaccum, "logicshellMuVetoIn");
  logicshellCuColdBox =
      new G4LogicalVolume(shellCuColdBox, Copper, "logicshellCuColdBox");
  logicSteelPlate = new G4LogicalVolume(SteelPlate, Steel, "logicSteelPlate");
  logicUnderSideBP =
      new G4LogicalVolume(UnderSideBP, BoratedPE, "logicUnderSideBP");
  logicUnderSideHDPE =
      new G4LogicalVolume(UnderSideHDPE, HDPE, "logicUnderSideHDPE");
  logicdetVol = new G4LogicalVolume(detVol, Vaccum, "logicdetVol");
  ...
  // when subtracting 2 boxes, the origin of the parent box from which
  // subtraction is done is retained in the resultant box thus all the boxes
  // above table are kept at (x,0,0)
  physshellMuVetoOut =
      new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshellMuVetoOut,
                        "physshellMuVetoOut", logicWorld, false, 1, true);
  physshell0 = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshell0,
                                 "physshell0", logicWorld, false, 2, true);
  physshell1 = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshell1,
                                 "physshell1", logicWorld, false, 3, true);
  physshell2 = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshell2,
                                 "physshell2", logicWorld, false, 4, true);
  physshell3 = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshell3,
                                 "physshell3", logicWorld, false, 5, true);
  physshell4 = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshell4,
                                 "physshell4", logicWorld, false, 6, true);
  physshell5 = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshell5,
                                 "physshell5", logicWorld, false, 7, true);
  physshell6 = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshell6,
                                 "physshell6", logicWorld, false, 8, true);
  physshell7 = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshell7,
                                 "physshell7", logicWorld, false, 9, true);
  physshellMuVetoIn =
      new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshellMuVetoIn,
                        "physshellMuVetoIn", logicWorld, false, 10, true);
  physshellCuColdBox =
      new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicshellCuColdBox,
                        "physshellCuColdBox", logicWorld, false, 11, true);
  physSteelPlate = new G4PVPlacement(
      0, G4ThreeVector(xloc, 0., -1.0 * zoffset * cm), logicSteelPlate,
      "physSteelPlate", logicWorld, false, 12, true);
  physUnderSideBP = new G4PVPlacement(
      0, G4ThreeVector(xloc, 0., -1.0 * (zoffset + 6.25) * cm),
      logicUnderSideBP, "physUnderSideBP", logicWorld, false, 13, true);
  physUnderSideHDPE = new G4PVPlacement(
      0, G4ThreeVector(xloc, 0., -1.0 * (zoffset + 16.25) * cm),
      logicUnderSideHDPE, "physUnderSideHDPE", logicWorld, false, 14, true);
  physdetVol = new G4PVPlacement(0, G4ThreeVector(xloc, 0., 0.), logicdetVol,
                                 "physdetVol", logicWorld, false, 0, true);
}
...
G4VSolid *MyDetectorConstruction::SubtractBox(G4VSolid *sol,
                                              G4ThreeVector Boxsz,
                                              G4ThreeVector relPosSubVol) {
  G4VSolid *box = new G4Box("Box", 0.5 * Boxsz.x() * cm, 0.5 * Boxsz.y() * cm,
                            0.5 * Boxsz.z() * cm);
  G4VSolid *subbox =
      new G4SubtractionSolid("sol-box", sol, box, 0, relPosSubVol);
  return subbox;
}

G4VSolid *MyDetectorConstruction::SubtractBoxfrmShell(
    G4ThreeVector shellsz, double thickness, double offset, G4ThreeVector Boxsz,
    G4ThreeVector relPosSubVol) {
  G4VSolid *shell =
      ConstructShell(shellsz.x(), shellsz.y(), shellsz.z(), thickness, offset);
  G4VSolid *box = new G4Box("Box", 0.5 * Boxsz.x() * cm, 0.5 * Boxsz.y() * cm,
                            0.5 * Boxsz.z() * cm);
  G4VSolid *subboxfrmshell =
      new G4SubtractionSolid("shell-box", shell, box, 0, relPosSubVol);
  return subboxfrmshell;
}

G4VSolid *MyDetectorConstruction::SubtractBoxfrmShell(G4ThreeVector shellsz,
                                                      double thickness,
                                                      double offset,
                                                      G4ThreeVector Boxsz) {
  G4ThreeVector relPosSubVol =
      G4ThreeVector(0, 0, -0.5 * (shellsz.z() - Boxsz.z()) * cm);
  G4VSolid *shell =
      ConstructShell(shellsz.x(), shellsz.y(), shellsz.z(), thickness, offset);
  G4VSolid *box = new G4Box("Box", 0.5 * Boxsz.x() * cm, 0.5 * Boxsz.y() * cm,
                            0.5 * Boxsz.z() * cm);
...
G4VPhysicalVolume *MyDetectorConstruction::Construct() {
  // Reading the material ordering and thicknesses from mac file
