...
void MyDetectorConstruction::ConstructSingleSheet()
{
	...
	physdetVol = new G4PVPlacement(0, G4ThreeVector(xloc + 2.5 * cm, 0., 0.), logicdetVol, "physdetVol", logicWorld, false, 0, true);
}
...
G4VSolid *MyDetectorConstruction::ConstructShell(double xsz, double ysz, double zsz, double thickness, double offset = 0.0)
{
	...
	G4VSolid *shell = new G4SubtractionSolid("BoxOut-BoxIn", boxout, boxin, 0, G4ThreeVector(-1.0 * offset * cm, 0., 0.));
	return shell;
}
