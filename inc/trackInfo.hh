...
public:
  ...
  trackInformation();
  trackInformation(const G4Track *parentTrack, const G4Track *currentTrack,
                   Type type = Type::Copy);
  trackInformation(const trackInformation *aTrackInfo);
  virtual ~trackInformation();

  inline void *
  operator new(size_t); // this defines how the "new" operator will allocate
                        // memory for pointers of this type
  inline void operator delete(
      void *aTrackInfo); // this defines how the "delete" operator will clear
                         // memory for pointers of this type
  trackInformation &
  operator=(const trackInformation
                &right); // this defines the behaviour of the "=" operator
...
extern G4ThreadLocal G4Allocator<trackInformation>
    *aTrackInformationAllocator; // the allocator maintains a pool memory. it is
